...
async function fetchLiveData(pool: pg.Pool): Promise<{
  ...
  try {
    ...
    const signals = result.rows.map(row => ({
      ...
      confirm15Strict: row.confirm15_strict === 1 || row.confirm15_strict === 't' ? 1 : 0,
      confirm15Soft: row.confirm15_soft === 1 || row.confirm15_soft === 't' ? 1 : 0,
      sessionOk: row.session_ok === 1 || row.session_ok === 't' ? 1 : 0,
      sweepOk: row.sweep_ok === 1 || row.sweep_ok === 't' ? 1 : 0,
      trendOk: row.trend_ok === 1 || row.trend_ok === 't' ? 1 : 0,
      blockedByBtc: row.blocked_by_btc === 1 || row.blocked_by_btc === 't' ? 1 : 0,
      btcGate: row.btc_gate || null,
      gateScore: Number(row.gate_score) || 0,
      btcClose: Number(row.btc_close) || 0,
      btcVwap: Number(row.btc_vwap) || 0,
      btcEma200: Number(row.btc_ema200) || 0,
      btcRsi: Number(row.btc_rsi) || 0,
      btcDeltaVwap: Number(row.btc_delta_vwap) || 0,
      btcBull: row.btc_bull === 1 || row.btc_bull === 't' ? 1 : 0,
      btcBear: row.btc_bear === 1 || row.btc_bear === 't' ? 1 : 0,
      extManagedR: row.extmanagedr !== null ? Number(row.extmanagedr) : null,
      extRunnerExitReason: row.extrunnerexitreason || null,
      ...
      extTimeToTp1Seconds: row.exttimetotp1seconds ? Number(row.exttimetotp1seconds) : null,
      extTimeToTp2Seconds: row.exttimetotp2seconds ? Number(row.exttimetotp2seconds) : null,
      extTimeToStopSeconds: row.exttimetostopseconds ? Number(row.exttimetostopseconds) : null,
      rr: Number(row.rr) || 0,
      riskPct: Number(row.riskpct) || 0,
      confirm15m: row.confirm15m === 1 || row.confirm15m === 't' ? 1 : 0,
